...
//	console.log(flg)
	...
	$("audio").prop("src","static/img/snake.mp3");
	$(this).text("关闭背景音乐");
	flg=!flg;
}
...
$(".btn3").click(function(){
	console.log(localStorage.VIP);
	localStorage.setItem("VIP","666");
	console.log(localStorage.VIP);
	vipShow();
	console.log($("#bgpic img").css("opacity"));
	console.log(localStorage.VIP);
});

function vipShow(){
	if(typeof(Storage)!=="underfined"){
				if(localStorage.VIP=="666"){
					console.log(localStorage.VIP);
					$("#bgpic img").css("opacity",1);
				}
			}
...
//$(".btn4").click(function(){
//	$("<audio src='static/img/snake.mp3' autoplay='autoplay'/></audio>").appendTo("body");
//})
...
function Snake() {
	...
	$("#range").change(function(event){
					...
					var sudu=document.getElementById("range").value;//获取range的值
					//console.log(sudu);
					this.time=sudu*1000;//根据range值设置时间
//					console.log(this.time);
					clearInterval(_this.timer);//清除move中的定时器
					_this.timer = setInterval(addMove,this.time);//重启move中的定时器
				});
	...
	this.drawFood = function() {
		...
		var foodLizi=addFood();//获得addfood函数中返回的数组对象
		var foodTypeImg=foodLizi.img;//
		var foodScore=foodLizi.score;
           ...
           $(".dead").slideDown(300);
			...
			if(typeof(Storage)!=="underfined"){
				if(localStorage.score<_this.score){
					...
					document.getElementById("score-max").innerHTML = "贪吃蛇最高分："+localStorage.getItem("score");
				}
...
//				console.log("*********eaten***********");
				...
				var scoreViewI=document.getElementById("scoreView-i");
				scoreViewI.innerHTML=_this.score;
				//清空食物数组
				_this.foodList = [];
